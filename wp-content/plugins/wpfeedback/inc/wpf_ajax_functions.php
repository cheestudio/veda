            ...
            if ($post_id) {
                ...
                if ( ! empty( $wpf_task_tags_obj ) && ! is_wp_error( $wpf_task_tags_obj ) ) {
                    foreach ( $wpf_task_tags_obj as $term ) {
                        $response['wpf_tags'] .= '<span class="wpf_tag_name '.$term->slug.'">'.$term->name.'<a href="javascript:void(0)" onclick="wpf_delete_tag(\''.$term->name.'\',\''.$term->slug.'\','.$post_id.')" data-tag_slug="'.$term->slug.'"><i class="fa fa-times-circle"></i></a></span>';
                    }
...
if (!function_exists('wpf_create_graphics')) {
    function wpf_create_graphics(){
        ...
        if($graphics_id){
            ...
            echo $output .= '<div class="wpf-col-3"><a target="_blank" href="'.get_permalink($graphics_id).'"><div class="wpf_graphics_thumb" style="background-image:url('.get_the_post_thumbnail_url($graphics_id,'large').')"><div class="wpf_graphics_title">'.$graphics_name.'</div></div></a></div>';
        }
        exit;
    }
