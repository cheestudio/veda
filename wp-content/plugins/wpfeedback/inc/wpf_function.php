...
if (!function_exists('wpf_license_key_check_item')) {
    function wpf_license_key_check_item($wpf_license_key)
    {
        ...
        $cl = '';
        $handle = curl_init();
        $url = EDD_SL_STORE_URL."?edd_action=check_license&item_id=" . EDD_SL_ITEM_ID . "&license=$wpf_license_key&url=$site_url";
        // Set the url
...
//        print_r($outputObject); exit;
        if ($outputObject->license == 'valid') {
            $response = array('license' => $outputObject->license, 'expires' => $outputObject->expires, 'payment_id' => $outputObject->payment_id, 'checksum' => $outputObject->checksum);
        } else {
            $response = array('license' => $outputObject->license, 'expires' => '');
        }
        return $response;
    }
}
...
if (!function_exists('wpf_license_key_license_item')) {
    function wpf_license_key_license_item($wpf_license_key)
    {
        ...
        $cl = '';
        $handle = curl_init();
        $url = EDD_SL_STORE_URL."?edd_action=activate_license&item_id=" . EDD_SL_ITEM_ID . "&license=$wpf_license_key&url=$site_url";
        // Set the url
        curl_setopt($handle, CURLOPT_URL, $url);
        // Set the result output to be a string.
        curl_setopt($handle, CURLOPT_RETURNTRANSFER, true);
