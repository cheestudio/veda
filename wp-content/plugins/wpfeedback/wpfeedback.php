...
if (is_admin() && (!defined('DOING_AJAX') || !DOING_AJAX)) {
    ...
    if($wpf_license == 'valid' ){
        ...
        if ($outputObject['license'] == 'valid') {
            ...
            delete_option('wpf_decr_key',true);
            delete_option('wpf_decr_checksum',true);
            update_option('wpf_license', $outputObject['license'],'no');
        }
    }
...
//    $comment_count = $comment_count_obj->publish + 1;
    ...
    if (isset($_REQUEST['page'])) {
        if ($_REQUEST['page'] == 'wpfeedback_page_settings' || $_REQUEST['page'] == 'wpfeedback_page_tasks' || $_REQUEST['page'] == 'wpfeedback_page_integrate' || $_REQUEST['page'] == 'wpfeedback_page_upgrade' || $_REQUEST['page'] == 'wpfeedback_page_support' || $_REQUEST['page'] == 'wpfeedback_page_permissions') {
            ?>
                ...
                function wp_feedback_cat_filter() {
                    ...
                    jQuery.ajax({
                        ...
                        data: {
                            ...
                            wpf_nonce:wpf_nonce,
                            task_types: selected_task_types_values,
                    ...
                    jQuery.ajax({
                        ...
                        success: function (data) {
                            ...
                            if(current_user_id == post_author_id || wpf_user_type == 'advisor'){
                                jQuery('#wpf_delete_task_container').html('<a href="javascript:void(0)" class="wpf_task_delete_btn"><i class="fa fa-trash-alt"></i> '+wpf_delete_ticket+'</a><p class="wpf_hide" id="wpf_task_delete">'+wpf_delete_conform_text2+' <a href="javascript:void(0);" class="wpf_task_delete" data-taskid='+ post_id +' data-elemid='+post_task_no+'>'+wpf_yes+'</a></p>');
                            }else{
...
function show_wpf_comment_button()
{
    ...
    $current_user_name = addslashes($current_user_name);
    $current_page_url = get_permalink($wp_query->post->ID);
    $current_page_title = addslashes(get_the_title($wp_query->post->ID));
...
//    $comment_count = $comment_count_obj->publish + 1;
    ...
    if($current_user_id>0){
        ...
        $wpf_go_to_dashboard_btn = '<div class="wpf_sidebar_dashboard"><a href="'.admin_url().'admin.php?page=wpfeedback_page_tasks" target="_blank">'.__('Go to the Dashboard','wpfeedback').' <i class="fa fa-columns"></i></a></div>';
        $wpf_go_to_dashboard_btn.='<div class="wpf_report_trigger">';
        if($wpf_daily_report=='yes'){
            $wpf_go_to_dashboard_btn.='<a href="javascript:wpf_send_report(\'daily_report\')"><i class="fa fa-envelope"></i> '.__('Daily Report','wpfeedback').'</a>';
        }
        if($wpf_weekly_report=='yes'){
            $wpf_go_to_dashboard_btn.='<a href="javascript:wpf_send_report(\'weekly_report\')"><i class="fa fa-envelope"></i> '.__('Weekly Report','wpfeedback').'</a>';
        }
    ...
    $wpf_currentpage_task_filter_btn = '<div class="wpf_sidebar_filter wpf_col2">
        <!-- =================Current page filter Tabs================-->
        <button class="wpf_filter wpf_currentpage_filter_taskstatus" onclick="wpf_show(\'wpf_currentpage_filter_taskstatus\')">'.__("Task Status","wpfeedback").'</button>
        <button class="wpf_filter wpf_currentpage_filter_taskpriority" onclick="wpf_show(\'wpf_currentpage_filter_taskpriority\')">'.__("Task Urgency","wpfeedback").'</button></div>';

    $wpf_allpage_task_filter_btn = '<div class="wpf_sidebar_filter wpf_col2">
        <!-- =================All page filter Tabs================-->
        <button class="wpf_filter wpf_allpage_filter_taskstatus" onclick="wpf_show(\'wpf_allpage_filter_taskstatus\')">'.__("Task Status","wpfeedback").'</button>
        <button class="wpf_filter wpf_allpage_filter_taskpriority" onclick="wpf_show(\'wpf_allpage_filter_taskpriority\')">'.__("Task Urgency","wpfeedback").'</button></div>';

    $wpf_backend_task_filter_btn = '<div class="wpf_sidebar_filter wpf_col2">
        <!-- =================Admin page filter Tabs================-->
        <button class="wpf_filter wpf_backend_filter_taskstatus" onclick="wpf_show(\'wpf_backend_filter_taskstatus\')">'.__("Task Status","wpfeedback").'</button>
        <button class="wpf_filter wpf_backend_filter_taskpriority" onclick="wpf_show(\'wpf_backend_filter_taskpriority\')">'.__("Task Urgency","wpfeedback").'</button></div>';
    ...
    if ( $wpf_active == 1 && $wpf_check_page_builder_active == 0){
        ...
        if($disable_for_admin == 0){
            echo '<div id="wpf_already_comment" class="wpf_hide"><p class="wpf_btn">'.__('Task already exist for this element','wpfeedback').'</p></div><div id="wpf_launcher" data-html2canvas-ignore="true" ><div class="wpf_launch_buttons"><div class="wpf_start_comment"><a href="javascript:enable_comment();" title="'.__('Click to give your feedback!','wpfeedback').'" data-placement="left" class="comment_btn"><i class="fa fa-plus"></i></a></div>
            ...
            <!-- =================Top Tabs================-->
            <button class="wpf_tab_sidebar wpf_thispage wpf_active" onclick="openWPFTab(\'wpf_thispage\')" >'.__('On This Page','wpfeedback').'</button>
            ...
            <div class="wpf_sidebar_footer ' . $wpf_powered_class . '"><a href="https://wpfeedback.co/powered" target="_blank">'.__('Powered by','wpfeedback').' <img alt="" src="'.WPF_PLUGIN_URL.'images/WPFeedback-icon.png" /> <span>WPFeedback</span></a></div>
            </div>
            </div>
    ...
    $wpf_enabled = get_option('wpf_enabled');
    $wpf_allow_guest = get_option('wpf_allow_guest');
    if((!is_user_logged_in() && $wpf_enabled == 'yes') || ($wpf_enabled == 'yes' && $wpf_allow_guest == 'yes' )){
