...
function enable_comment(){
    ...
    if(wpf_reconnect == true){
        ...
        jQuery('body').css('cursor','url("'+wpf_reconnect_icon+'"), crosshair');
        jQuery('a').css('cursor','url("'+wpf_reconnect_icon+'"), crosshair');
    }
    else{
            ...
            var new_task_screenshot_obj = jQuery.extend({}, task_screenshot);
                ...
                jQuery.ajax({
                    ...
                    success: function(data){
                        jQuery('.wpf_loader_'+id).hide();
                        console.log('Upload successfully');
                        var comment_html = '<li class="wpf_author"><level class="task-author">'+current_user_name +'<span>'+wpf_just_now+'</span></level><div class="meassage_area_main"><a href="'+data+'" target="_blank"><img src="'+data+'" title="WPF Screenshot" alt="WPF Screenshot"/></a></div></li>';
                        jQuery('#task_comments_'+id).append(comment_html);
                        jQuery('#task_comments_'+id).animate({scrollTop: jQuery('#task_comments_'+id).prop("scrollHeight")}, 2000);
                    }
                });
...
function generate_wpfb_task_html(wpfb_task_id,wpfb_metas){
    ...
    var wpf_task_status_label= '<div class="wpf_task_label"><span class="task_status wpf_'+wpfb_metas.task_status+'" title="Status: '+wpfb_metas.task_status+'"><i class="fas fa-thermometer-half"></i></span>';
    var wpf_task_priority_label= '<span class="priority wpf_'+wpfb_metas.task_priority+'" title="Priority: '+wpfb_metas.task_priority+'"><i class="fas fa-exclamation-circle"></i></span></div>';
    //var wpfb_tags_html = '<div class="wpf_task_tags"><span class="wpf_task_tag">Test tag</span><span class="wpf_task_tag_more">...</span></div>';
...
function wpf_task_popover_html(wpf_task_type,comment_count,wpfb_task_id,wpfb_metas,wpfb_users_html,wpfb_task_priority_html,wpfb_task_status_html,wpfb_messages_html) {
    ...
    if(wpf_tab_permission.priority==true){
        tab_nav_html+='<li class="nav-item" title="Status"><a class="nav-link wpf_prio_tab" id="wpfbpriority-tab-'+comment_count+'" data-toggle="tab" href="#wpfbpriority-'+comment_count+'" role="tab" aria-controls="wpfbpriority" aria-selected="false"><i class="fa fa-exclamation-circle"></i></a></li>';
    }
    if(wpf_tab_permission.status==true){
        tab_nav_html+='<li class="nav-item" title="Priority"><a class="nav-link wpf_stat_tab" id="wpfbtaskstatus-tab-'+comment_count+'" data-toggle="tab" href="#wpfbtaskstatus-'+comment_count+'" role="tab" aria-controls="wpfbtaskstatus" aria-selected="false"><i class="fa fa-thermometer-half"></i></a></li>';
    }
    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            jQuery.each(k,function (index, value) {
                ...
                var wpf_task_status_label= '<div class="wpf_task_label"><span class="task_status wpf_'+onload_wpfb_tasks[value].task_status+'" title="Status: '+onload_wpfb_tasks[value].task_status+'"><i class="fas fa-thermometer-half"></i></span>';
                var wpf_task_priority_label= '<span class="priority wpf_'+onload_wpfb_tasks[value].task_priority+'" title="Priority: '+onload_wpfb_tasks[value].task_priority+'"><i class="fas fa-exclamation-circle"></i></span></div>';
                //var wpfb_tags_html = '<div class="wpf_task_tags"><span class="wpf_task_tag">Test tag</span><span class="wpf_task_tag_more">...</span></div>';
            ...
            success: function (data) {
                ...
                if(response.status==1){
                    ...
                    if(response.type==1){
                        ...
                        var comment_html = '<li class="wpf_author"><level class="task-author">'+current_user_name+'<span>'+wpf_just_now+'</span></level><div class="meassage_area_main"><a href="'+response.message+'" download="'+wpf_download_file+'"><i class="fa fa-download" aria-hidden="true"></i> '+ wpf_download_file+'</a></div></li>';
                    }
